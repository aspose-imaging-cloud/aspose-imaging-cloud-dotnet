...
namespace Aspose.Imaging.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeImagingCloudContext.atcc = new AsposeImagingCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeImagingCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeImagingCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
